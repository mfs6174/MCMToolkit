...
void mfs(int x,int p,int ti,D &l,int z)
{
  ...
  if (l.w==0)
  {
    ...
    for (i=l.m+1;i<=n;i++)
    {
      tt+=3;
      pp++;
      if (p+suan(l.z,pp)<f[x+1][ch[l.z].l[i]])
      {
        f[x+1][ch[l.z].l[i]]=p+suan(l.z,pp);
        xian[x+1][ch[l.z].l[i]]=l.z;
        zhan[x+1][ch[l.z].l[i]]=z;
        ot[x+1][ch[l.z].l[i]]=ti+tt;
      }
      else
        if (f[x+1][ch[l.z].l[i]]==p+suan(l.z,pp))
        {
          if (ti+tt<ot[x+1][ch[l.z].l[i]])
          {
            ot[x+1][ch[l.z].l[i]]=ti+tt;
            xian[x+1][ch[l.z].l[i]]=l.z;
            zhan[x+1][ch[l.z].l[i]]=z;
  ...
  else
  {
    ...
    for (i=l.m+1;i<=n;i++)
    {
      tt+=3;
      pp++;
      if (p+suan(l.z,pp)<f[x+1][ch[l.z].ll[i]])
      {
        f[x+1][ch[l.z].ll[i]]=p+suan(l.z,pp);
        xian[x+1][ch[l.z].ll[i]]=-l.z;
        zhan[x+1][ch[l.z].ll[i]]=z;
        ot[x+1][ch[l.z].ll[i]]=ti+tt;
      }
      else
        if (f[x+1][ch[l.z].ll[i]]==p+suan(l.z,pp))
        {
          if (ti+tt<ot[x+1][ch[l.z].ll[i]])
          {
            ot[x+1][ch[l.z].ll[i]]=ti+tt;
            xian[x+1][ch[l.z].ll[i]]=-l.z;
            zhan[x+1][ch[l.z].ll[i]]=z;
      ...
      for (j=1;j<=9999;j++)
        if (f[i][j]!=maxlongint)
          for (k=0;k<hash[j].size();k++)
            mfs(i,f[i][j],ot[i][j],hash[j][k],j);
    for (i=2;i<=dp+1;i++)
    {
      if (f[i][zhong]<10000)
      {
        ouf<<i-1<<endl;
        ouf<<f[i][zhong]<<' '<<ot[i][zhong]-5<<endl;
        dayin(i,zhong);
        ouf<<zhong<<endl<<endl;
      }
    }
